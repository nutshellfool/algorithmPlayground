...
public class BinarySearchTree {
  ...
  private TreeNode findMin(TreeNode node) {
    ...
    return leftNode == null ? node : findMin(leftNode.leftChild);
  }
   ...
   *
   * <p>Time and space Analysis: 1. the time complexity: because the first 2 lines is constants
   * complexity(O(1)) the last return line turn the problem into 2 part sub-problems so use the
   * Big-Oh additional rule, we got the T(n) = 2*O(1) + O(n/2) + O(n/2) = 2O(n/2) + 2*O(1) therefore
   * we get conclusion: the time complexity is O(lgn)
   *
   * <p>2. space complexity: no more auxiliary space for this algorithm
   *
   ...
   *
   * <p>Time and space Analysis:
   *
   * <p>1. the time complexity first the code structure is not recurrence structure, and in a
   * continue flow, so it's ok to apply the Big-Oh additional rule. first part is the tree inorder
   * traversal complexity, time complexity is (O(n)) second part is also the traversal complexity,
   * at least time complexity is (O(n))
   *
   * <p>therefore the time complexity is T(n) = O(n) +O(n) we get the conclusion : O(n)
   *
