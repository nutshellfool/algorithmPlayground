...
class LeetCodeBinarySearchSolution {
  ...
  double myPowRecursion(double x, int n) {
    return _myPowRecursion(x, n);
  }

  private double _myPowRecursion(double x, long n) {
    if (n == 0) {
      return 1.0;
    }
    ...
    if (n < 0) {
      return _myPowRecursion(1.0 / x, -n);
    }

    return ((n & 1) == 1) ? _myPowRecursion(x * x, n / 2) * x
        : _myPowRecursion(x * x, n / 2);
  }
